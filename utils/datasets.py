...
__all__ = [
    'crop_image_CHW', 'PatchDataset_NCHW', 'NHWC2NCHW_normalize', 'NHWC2NCHW',
    'save_binary', 'load_binary', 'makedirpath', 'task',
    'DictionaryConcatDataset', 'to_device', 'distribute_scores', 'resize'
]
...
class DictionaryConcatDataset(Dataset):
    def __init__(self, d_of_datasets):
        ...
        assert min(lengths) == max(
            lengths), 'Length of the datasets should be the same'
...
def distribute_scores(score_masks, output_shape, K: int, S: int) -> np.ndarray:
    ...
    results = [
        distribute_score(score_masks[n], output_shape, K, S) for n in range(N)
    ]
