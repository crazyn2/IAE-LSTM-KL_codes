...
def make_mvtec_dataset(root: str | Path,
    ...
    root = Path(root)
    samples_list = [(str(root), ) + f.parts[-3:] for f in root.glob(r"**/*")
    ...
    samples[
        "image_path"] = samples.path + "/" + samples.split + "/" + samples.label + "/" + samples.image_path
    ...
    samples.loc[(samples.split == "test") &
                ...
                "mask_path"] = mask_samples.image_path.values

    # assert that the right mask files are associated with the right test images
    if len(samples.loc[samples.label_index == LabelName.ABNORMAL]):
        assert (
            samples.loc[samples.label_index == LabelName.ABNORMAL].apply(
...
class MVTec(AnomalibDataModule):
    ...
    def __init__(
        ...
        self.train_data = MVTecDataset(task=task,
                                       transform=transform_train,
                                       split=Split.TRAIN,
                                       root=root,
                                       category=category)
        self.test_data = MVTecDataset(task=task,
